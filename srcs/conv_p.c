...
/*   Updated: 2017/09/26 16:36:40 by achambon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		print_p(t_printf *pf, t_conv *conv, int len, char *str)
{
	if(conv->min_width && (!(conv->precision)) && conv->min_width > len)
		option_p(&*pf, conv->min_width - len - 3, ' ', conv);
	buffer(&*pf, "0x", 2);
...
// int		print_p(t_printf *pf, t_conv *conv, int len, char *str)
// {
// 	if(conv->min_width && (!(conv->precision)) && conv->flag & PRECISION && conv->min_width > len)
// 		option_p(&*pf, conv->min_width - len - 2, ' ', conv);
// 	else if (conv->min_width > len)
// 		option_p(&*pf, conv->min_width - pf->len, ' ', conv);
// 	buffer(&*pf, "0x", 2);
// 	if (conv->precision && conv->precision < conv->min_width && conv->min_width > len)
// 		option_p(&*pf, conv->min_width - conv->precision - 2, '0', conv);
// 	if(conv->min_width <= conv->precision)
