...
int		conv_d(t_printf *pf, t_conv *conv)
{
	...
	(conv->flag & MODIFIER_H ? apint = (intmax_t)((short)va_arg(pf->ap, int)) : 0);
	(conv->flag & MODIFIER_HH ? apint = (intmax_t)((char)va_arg(pf->ap, int)) : 0);
	...
	((str[0] == '-') && (conv->flag & MINUS)) ? ((flag_hh = 1) && (conv->min_width--)) : 0;
//JUSTE LARGEUR MINIMALE
	if (conv->min_width && !conv->precision)
